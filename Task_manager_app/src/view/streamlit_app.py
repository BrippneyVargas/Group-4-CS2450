...
class TaskManager:
    ...
    # Save_tasks method here
    def save_tasks(self):
        """Save all tasks to persistent storage via FastAPI."""
        with st.spinner("Saving tasks..."):
            try:
                response = requests.post("http://localhost:8000/save_tasks")
                response.raise_for_status()
     ...
     # load_tasks method
    def load_tasks(self):
        """Load tasks from the API or from saved file."""
        with st.spinner("Loading tasks..."):
            try:
                # Try to load tasks from saved file first
                load_response = requests.get("http://localhost:8000/load_tasks")
                load_response.raise_for_status()
                # Then fetch the current tasks
                self.fetch_tasks()
                return True
            except requests.RequestException as e:
...
class TaskUI:
    ...
    # Add display_save_load_buttons method here
    def display_save_load_buttons(self):
        """Display buttons for saving and loading tasks."""
        cols = st.columns([3, 1, 1])
        with cols[0]:
            if self.task_manager.api_available:
                st.success("✅ API Connected")
            else:
                st.warning("⚠️ API Offline - Using Local Storage")
        with cols[1]:
    ...
    def run(self):
        ...
        self.display_title()
        self.display_save_load_buttons()  # Add this line to call the new method
...
if __name__ == "__main__":
    main()
>>>>>>> ef57cad24a2bf47c3039491480546ea055c5a855
=======
"""
...
if __name__ == "__main__":
    main()
>>>>>>> fe7ec86e7c2d386b39ce926a4a63555419ae1205
