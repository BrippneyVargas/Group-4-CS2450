...
class Styler:
    ...
    def __init__(self) -> None:
        ...
        os.chdir(os.path.join("Task_manager_app", "src"))
        self.__config_object.read("config.ini")
        os.chdir(os.path.join("..", ".."))

    def apply_custom_theme(self, dark_mode=True) -> None: #Change the two modes to a composition.
        """
        ...
        """
        priority_info = self.__config_object["PRIORITY"]
        theme_info = None
        if dark_mode:
            theme_info = self.__config_object["DARKMODE"]
            darkmode_css = f"""
            <style>
            body, .stApp {{ overflow: hidden !important; max-height: 100vh; }}
            .stApp {{ background-color: {theme_info["background_color"]}; max-width: 100vw !important; margin: 0; padding: 0; }}
            ::-webkit-scrollbar {{ display: none; }}
            .priority-high {{ background-color: {priority_info["high_priority"]}; color: white; padding: 5px 10px; border-radius: 5px; text-align: center; }}
            .priority-medium {{ background-color: {priority_info["medium_priority"]}; color: white; padding: 5px 10px; border-radius: 5px; text-align: center; }}
            .priority-low {{ background-color: {priority_info["low_priority"]}; color: black; padding: 5px 10px; border-radius: 5px; text-align: center; }}
            .footer {{ position: fixed; left: 0; bottom: 0; width: 100%; text-align: center; padding: 20px 0; color: {theme_info["primary_light"]}; font-size: 0.9em; background-color: {theme_info["background_color"]}; }}
            ...
            theme_info = self.__config_object["LIGHTMODE"]
            lightmode_css = f"""
            <style>
            ...
            div.stSelectbox label {{ background-color: {theme_info["background_color"]}; color: {theme_info["text_color"]}; }}
            ...
            div.stTextArea label {{ background-color: {theme_info["background_color"]}; color: {theme_info["text_color"]}; }}
            div.stTextArea textarea {{background-color: white; color: black}}
            div.stRadio > label {{ background-color: {theme_info["background_color"]}; color: {theme_info["text_color"]}; }}     
            .priority-high {{ background-color: {priority_info["high_priority"]}; color: black; padding: 5px 10px; border-radius: 5px; text-align: center; }}
            .priority-medium {{ background-color: {priority_info["medium_priority"]}; color: black; padding: 5px 10px; border-radius: 5px; text-align: center; }}
            .priority-low {{ background-color: {priority_info["low_priority"]}; color: black; padding: 5px 10px; border-radius: 5px; text-align: center; }}
